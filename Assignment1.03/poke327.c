...
#define IF INT_MAX
int move_cost[num_character_types][num_terrain_types] = {
  {IF, IF, 10, 10, 10, 20, 10, IF, IF, IF, 10},
  {IF, IF, 10, 50, 50, 15, 10, 15, 15, IF, IF},
  {IF, IF, 10, 50, 50, 20, 10, IF, IF, IF, IF},
  {IF, IF, IF, IF, IF, IF, IF, IF, IF, 7, IF},
  {IF, IF, 10, 50, 50, 20, 10, IF, IF, IF, IF}
};
...
static void print_map()
{
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      ...
      if(world.pc.position[dim_y] == y && world.pc.position[dim_x] == x){
        putchar('@');
...
static int32_t hiker_cmp(const void *key, const void *with) {
  return (world.hiker_distance[((path_t *) key)->pos[dim_y]][((path_t *) key) -> pos[dim_x]] 
  - world.hiker_distance[((path_t *) with) -> pos[dim_y]][((path_t *) with) -> pos[dim_x]]);
}

static int32_t rival_cmp(const void *key, const void *with) {
  return (world.rival_distance[((path_t *) key)->pos[dim_y]][((path_t *) key) -> pos[dim_x]] 
  - world.rival_distance[((path_t *) with) -> pos[dim_y]][((path_t *) with) -> pos[dim_x]]);
}

#define ter_cost(ch, x, y) move_cost[ch][m->map[y][x]]

void dist_cal(map_t *m){
  static path_t path[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      world.hiker_distance[y][x] = world.rival_distance[y][x] = INT_MAX;
    }
  }
  world.hiker_distance[world.pc.position[dim_y]][world.pc.position[dim_x]] = 0;
  world.rival_distance[world.pc.position[dim_y]][world.pc.position[dim_x]] = 0;
  ...
  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      //path[y][x].hn = heap_insert(&h, &path[y][x]);
      if(ter_cost(char_hiker, x, y) != INT_MAX){
        path[y][x].hn = heap_insert(&h, &path[y][x]);
      }else{
        path[y][x].hn = NULL;
      }
...
//hiker's 8-directions 
  while ((p = heap_remove_min(&h))) {
    p->hn = NULL;

    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (world.hiker_distance[p->pos[dim_y] - 1][p->pos[dim_x]    ] >
         world.hiker_distance[p->pos[dim_y]][p->pos[dim_x]] +
         ter_cost(char_hiker, p->pos[dim_x], p->pos[dim_y]))) {
    ...
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) &&
        (world.hiker_distance[p->pos[dim_y]    ][p->pos[dim_x] - 1] >
         world.hiker_distance[p->pos[dim_y]][p->pos[dim_x]] +
         ter_cost(char_hiker, p->pos[dim_x], p->pos[dim_y]))) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) &&
        (world.hiker_distance[p->pos[dim_y] - 1][p->pos[dim_x] - 1] >
         world.hiker_distance[p->pos[dim_y]][p->pos[dim_x]] +
         ter_cost(char_hiker, p->pos[dim_x], p->pos[dim_y]))) {
      ...
      world.hiker_distance[p->pos[dim_y] - 1][p->pos[dim_x] - 1] =
        ...
        ter_cost(char_hiker, p->pos[dim_x], p->pos[dim_y]);
      heap_decrease_key_no_replace(&h,
                                   path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn);
  ...
  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      //path[y][x].hn = heap_insert(&h, &path[y][x]);
      if(ter_cost(char_rival, x, y) != INT_MAX){
        path[y][x].hn = heap_insert(&h, &path[y][x]);
      }else{
        path[y][x].hn = NULL;
      }
...
//rival's 8-directions 
  while ((p = heap_remove_min(&h))) {
    p->hn = NULL;

    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (world.rival_distance[p->pos[dim_y] - 1][p->pos[dim_x]    ] >
         world.rival_distance[p->pos[dim_y]][p->pos[dim_x]] +
         ter_cost(char_rival, p->pos[dim_x], p->pos[dim_y]))) {
    ...
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) &&
        (world.rival_distance[p->pos[dim_y]    ][p->pos[dim_x] - 1] >
         world.rival_distance[p->pos[dim_y]][p->pos[dim_x]] +
         ter_cost(char_rival, p->pos[dim_x], p->pos[dim_y]))) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) &&
        (world.rival_distance[p->pos[dim_y] - 1][p->pos[dim_x] - 1] >
         world.rival_distance[p->pos[dim_y]][p->pos[dim_x]] +
         ter_cost(char_rival, p->pos[dim_x], p->pos[dim_y]))) {
...
void print_hiker_distance(){
  int x, y;

  for(y = 0; y < MAP_Y; y++){
    for(x = 0; x < MAP_X; x++){

      if(world.hiker_distance[y][x] != INT_MAX){
        printf(" %02d", world.hiker_distance[y][x] % 100);
...
void print_rival_distance(){
  int x, y;

  for(y = 0; y < MAP_Y; y++){
    for(x = 0; x < MAP_X; x++){

      if(world.rival_distance[y][x] != INT_MAX){
        printf(" %02d", world.rival_distance[y][x] % 100);
...
void place_pc(){
  ...
  world.pc.position[dim_x] = x;
  world.pc.position[dim_y] = y;
...
int main(int argc, char *argv[])
{
  ...
  do {
    ...
    place_pc();
    dist_cal(world.cur_map);

    print_map();  
    print_hiker_distance();
    print_rival_distance();
