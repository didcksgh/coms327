...
int get_dmg(pokemon *p_attack, pokemon *p_defense, int i)
{
  // If the move has no power (status moves), do no damage.
  if (moves[i].power <= 0 || moves[i].power == INT_MAX) {
    return 0;
  }

  // Accuracy check: if accuracy is defined and we fail, return 0 damage (miss).
  ...
  // Base damage part: ((2 * Level) / 5 + 2)
  double level = (double) p_attack->get_level();
  double base = (2.0 * level) / 5.0 + 2.0;

  // Choose correct attack/defense stats depending on damage class.
  double A, D;
  if (moves[i].damage_class_id == 2) {        // Physical
    A = (double) p_attack->get_atk();
    D = (double) p_defense->get_def();
  } else if (moves[i].damage_class_id == 3) { // Special
    A = (double) p_attack->get_spatk();
    D = (double) p_defense->get_spdef();
  } else {
    // Non-damaging move types, just in case
    return 0;
  }

  double power = (double) moves[i].power;

  // Core damage formula: (((base * power * A / D) / 50) + 2)
  double damage = base * power * (A / D);
  damage = damage / 50.0 + 2.0;

  // Critical hit (same condition you tried before)
  double critical = 1.0;
  if (rand() % 256 < (p_attack->s->base_stat[stat_speed] / 2)) {
    critical = 1.5;
  }
  ...
  double stab = 1.0;
  if (p_attack->pt && p_attack->pt->type_id == moves[i].type_id) {
    stab = 1.5;
  }

  // Random factor: between 0.85 and 1.00
  double random = (rand() % 16 + 85) / 100.0;  // 85–100 inclusive → 0.85–1.00

  // Type effectiveness not implemented: keep as 1.0 for this assignment
  double type = 1.0;

  damage = damage * critical * stab * random * type;
  ...
  clear();
    ...
    switch (input){
      case '1':
        ...
        normal_fight(mp, 0, 0, pc_move, npc_move, pc_p, npc_np);
        break;
      case '2':
        int item;
        item = show_bag();
        npc_move = get_trainers_move(npc_np);
        npc_damage = get_dmg(npc_np, pc_p, npc_move);
        switch (item){
          case '1':
              if(world.pc.num_rev >0){
                if(world.pc.buddy[0]){
                  choose_revive();
                  item_used_fight(npc_damage, npc_move, pc_p, npc_np);
                }
...
void io_encounter_pokemon()
{
  ...
  //Main wild battle loop
  while(!battle_over && !world.pc.is_battle_end()) {
    //check if the wild pokemon is already fainted
    if(p->get_hp() <= 0){
      clear();
    ...
    //check if our active pokemon fainted
    if(pc_p->get_hp() <= 0){
      clear();
      mvprintw(0,0,"%s has been fainted!", pc_p->get_species());
      mvprintw(1,0, "Press any key to continue...");
      ...
      if(world.pc.is_battle_end()) {
        clear();
        mvprintw(0, 0, "ALL of your Pokemon have fainted");
        mvprintw(1, 0, "Press any key to continue...");
        refresh();
        getch();
        battle_over = true;
    ...
    switch (input){
      ...
      case '1': {
        ...
        normal_fight(mp, 0, 0, pc_move, npc_move, pc_p, p);
        break;
      }
        ...
        switch (item){
          ...
          case '1': {
              if(world.pc.num_rev > 0){
                ...
                //wild's turn after item use
                npc_move = get_trainers_move(p);
                npc_damage = dmg_cal(p, npc_move);
                item_used_fight(npc_damage, npc_move, pc_p, p);
              }else{
                clear();
          ...
          case '3': {
            ...
            if(slot == -1) {
              //party full
              clear();
              mvprintw(0, 0, "You already have 6 pokemon in your pockeet!");
              mvprintw(1, 0, "The wild %s fled.", p->get_species());
              mvprintw(0, 1, "Press ant key to continue...");
              refresh();
              getch();

              world.pc.num_pokeball--;
              delete p;
              wild_deleted = true;
              battle_over = true;
              ...
              clear();
              mvprintw(0, 0, "You threw a Pokeball!");
              mvprintw(1, 0, "you captured %s!", p->get_species());
              mvprintw(2, 0, "Press any key to continue...");
              refresh();
              getch();

              world.pc.num_pokeball--;
              world.pc.buddy[slot] = p;
      ...
      case '3': {
        clear();
        mvprintw(0, 0, "You ran away from the wild %s!", p->get_species());
        mvprintw(1, 0, "Press any key to continue...");
        refresh();
        getch();
      ...
      //pokemon(switch active)
      case '4': {

        //check if there's any other active pokemon
        ...
        if(!has_other) {
          clear();
          mvprintw(0, 0, "You have no left active Pokemon!");
          mvprintw(1, 0, "Press any key to continue...");
          refresh();
          getch();
          break;
        ...
        //after switching, wild gets a free move
        npc_move = get_trainers_move(p);
        npc_damage =  get_dmg(p, pc_p, npc_move);
        item_used_fight(npc_damage, npc_move, pc_p, p);
      ...
      default: {
        mvprintw(13, 0, "%c is not a valid input!", input);
        mvprintw(14, 0, "Press any key to continue...");
        refresh();
        getch();
        break;
      }
  ...
  //Safety reason
  //if we ended without catching or explicitly deleting, clean up
  if(!caught && !wild_deleted) {
    delete p;

  }

  //redraw overworld after the battle
  io_display();
...
void io_choose_starter()
{
  ...
  curs_set(1);
  do {
    mvprintw( 4, 20, "Before you choose a Pokemon, each of");
