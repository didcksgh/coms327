...
typedef struct trainers {
  character_type_t c_type;
  movement_type_t m_type;
  pair_t dir;
} trainers_t;
...
#define rand_dir(dir) {     \
  int r = rand() % 8;    \
  dir[0] = eight_dirs[r][0]; \
  dir[1] = eight_dirs[r][1]; \
}

void pathfind(map_t *m);

//hiker's move function
void move_hiker_function(character_t *c, pair_t dest)
{
  int min = INT_MAX;
  ...
  for (int i = d; i < 8 + d; i++) {
    if ((world.hiker_dist[c->pos[dim_y] + eight_dirs[i % 8][dim_y]]
                         [c->pos[dim_x] + eight_dirs[i % 8][dim_x]] <=
         min) &&
        !world.cur_map->c_map[c->pos[dim_y] + eight_dirs[i % 8][dim_y]]
                            [c->pos[dim_x] + eight_dirs[i % 8][dim_x]] &&
        c->pos[dim_x] + eight_dirs[i % 8][dim_x] != 0 &&
        c->pos[dim_x] + eight_dirs[i % 8][dim_x] != MAP_X - 1 &&
        c->pos[dim_y] + eight_dirs[i % 8][dim_y] != 0 &&
        c->pos[dim_y] + eight_dirs[i % 8][dim_y] != MAP_Y - 1) {
      dest[dim_x] = c->pos[dim_x] + eight_dirs[i % 8][dim_x];
      dest[dim_y] = c->pos[dim_y] + eight_dirs[i % 8][dim_y];
      min = world.hiker_dist[dest[dim_y]][dest[dim_x]];
    }
  }
}

//rival's move function
void move_rival_function(character_t *c, pair_t dest)
{
  int min = INT_MAX;
  int d = rand()%8;
  ...
  for (int i = d; i < 8 + d; i++) {
    if ((world.rival_dist[c->pos[dim_y] + eight_dirs[i % 8][dim_y]]
                         [c->pos[dim_x] + eight_dirs[i % 8][dim_x]] <
         min) &&
        !world.cur_map->c_map[c->pos[dim_y] + eight_dirs[i % 8][dim_y]]
                            [c->pos[dim_x] + eight_dirs[i % 8][dim_x]] &&
...
//pacer's move function.
//start with a direction til hit the terrain cannot pass
void move_pacer_function(character_t *c, pair_t dest)
{
  ...
  terrain = world.cur_map->map[c->pos[dim_y] + c->trainers->dir[dim_y]]
                        [c->pos[dim_x] + c->trainers->dir[dim_x]];

  //valid move
  if ((terrain == ter_path || terrain == ter_grass || terrain == ter_clearing) &&
      !world.cur_map->c_map[c->pos[dim_y] + c->trainers->dir[dim_y]]
                          [c->pos[dim_x] + c->trainers->dir[dim_x]]) {
    dest[dim_x] = c->pos[dim_x] + c->trainers->dir[dim_x];
    dest[dim_y] = c->pos[dim_y] + c->trainers->dir[dim_y];
  } 
  ...
  if ((terrain != ter_path && terrain != ter_grass && terrain != ter_clearing) ||
      world.cur_map->c_map[c->pos[dim_y] + c->trainers->dir[dim_y]]
                         [c->pos[dim_x] + c->trainers->dir[dim_x]]) {
    c->trainers->dir[dim_x] *= -1;
    c->trainers->dir[dim_y] *= -1;
  }

}

//have to stay in the territory spawned.
void move_wanderer_function(character_t *c, pair_t dest)
{
  dest[dim_x] = c->pos[dim_x];
  dest[dim_y] = c->pos[dim_y];

  //valid move
  if ((world.cur_map->map[c->pos[dim_y] + c->trainers->dir[dim_y]]
                         [c->pos[dim_x] + c->trainers->dir[dim_x]] ==
...
//sentry doesn't move
void move_sentry_function(character_t *c, pair_t dest)
{
  dest[dim_x] = c->pos[dim_x];
  dest[dim_y] = c->pos[dim_y];
}

//it moves like wanderer, but can cross the terrain
void move_explorer_function(character_t *c, pair_t dest)
{
  dest[dim_x] = c->pos[dim_x];
  dest[dim_y] = c->pos[dim_y];
  ...
  if ((move_cost[char_other][world.cur_map->map[c->pos[dim_y] +
                                                c->trainers->dir[dim_y]]
                                               [c->pos[dim_x] +
                                                c->trainers->dir[dim_x]]] !=
       INT_MAX) &&
      !world.cur_map->c_map[c->pos[dim_y] + c->trainers->dir[dim_y]]
                          [c->pos[dim_x] + c->trainers->dir[dim_x]]) {
    dest[dim_x] = c->pos[dim_x] + c->trainers->dir[dim_x];
  ...
  if ((move_cost[char_other][world.cur_map->map[c->pos[dim_y] +
                                               ...
                                               [c->pos[dim_x] +
                                                c->trainers->dir[dim_x]]] ==
       INT_MAX) || world.cur_map->c_map[c->pos[dim_y] + c->trainers->dir[dim_y]]
                                      [c->pos[dim_x] + c->trainers->dir[dim_x]]) {
    rand_dir(c->trainers->dir);
  }
...
//stay still
void move_pc_function(character_t *c, pair_t dest)
{
  dest[dim_x] = c->pos[dim_x];
  dest[dim_y] = c->pos[dim_y];
}
...
//array of function
void (*move_function[num_movement_types])(character_t *, pair_t) = {
  move_hiker_function,
  move_rival_function,
  move_pacer_function,
  move_wanderer_function,
  move_sentry_function,
  move_explorer_function,
  move_pc_function,
};

void random_pos(pair_t pos)
{
...
void new_hiker()
{
  ...
  do {
    random_pos(pos);
           ...
           world.cur_map->c_map[pos[dim_y]][pos[dim_x]]);

  c = malloc(sizeof (*c));
  c->trainers = malloc(sizeof (*c->trainers));

  c->pos[dim_y] = pos[dim_y];
  c->pos[dim_x] = pos[dim_x];

  c->trainers->c_type = char_hiker;
  c->trainers->m_type = move_hiker;

  c->trainers->dir[dim_x] = 0;
  c->trainers->dir[dim_y] = 0;
  ...
  world.cur_map->c_map[pos[dim_y]][pos[dim_x]] = c;
}
...
void new_rival()
{
  ...
  c->trainers->c_type = char_rival;
  c->trainers->m_type = move_rival;
...
void new_char_other()
{
  ...
  c->trainers->c_type = char_other;

  switch (rand() % 4) {
  case 0:
    c->trainers->m_type = move_pacer;
    c->symbol = PACER_SYMBOL;
    break;
  case 1:
    c->trainers->m_type = move_wanderer;
    c->symbol = WANDERER_SYMBOL;
    break;
  case 2:
    c->trainers->m_type = move_sentry;
    c->symbol = SENTRY_SYMBOL;
    break;
  case 3:
    c->trainers->m_type = move_explorer;
...
{
  //need at least one hiker and one rival
  int num_trainers = 2;
...
int32_t cmp_turns(const void *key, const void *with)
{
  if(((character_t *) key)->n_turn == ((character_t *) with)->n_turn){
      return ((((character_t *) key)->seq_num -((character_t *) with)->seq_num));

  }else{
    return ((((character_t *) key)->n_turn - ((character_t *) with)->n_turn));
  }
}
...
void init_pc()
{
  ...
  world.cur_map->c_map[y][x] = &world.pc;
  world.pc.n_turn = 0;
...
static int new_map()
{
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      world.cur_map->c_map[y][x] = NULL;
    }
  }

  heap_init(&world.cur_map->turn, cmp_turns, delete_character);
...
static void print_map()
{
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      if (world.cur_map->c_map[y][x]) {
        putchar(world.cur_map->c_map[y][x]->symbol);
      } else {
...
void game_loop()
{
  ...
  while (1) {
    ...
    if (c == &world.pc) {
      print_map();
      usleep(250000);
      c->n_turn += move_cost[char_pc][world.cur_map->map[c->pos[dim_y]]
                                                           [c->pos[dim_x]]];
    } else {
      move_function[c->trainers->m_type](c, d);
      world.cur_map->c_map[c->pos[dim_y]][c->pos[dim_x]] = NULL;
      world.cur_map->c_map[d[dim_y]][d[dim_x]] = c;
      c->n_turn += move_cost[c->trainers->c_type][world.cur_map->map[d[dim_y]]
                                                                 [d[dim_x]]];
      c->pos[dim_y] = d[dim_y];
      c->pos[dim_x] = d[dim_x];
    }
