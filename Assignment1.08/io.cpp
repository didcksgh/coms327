...
void io_pokemon_encounter(){
  ...
  if(pe == 0){
    ...
    mvprintw(0,0, "Wild %s appear!", new_pokemon.identifier);
    mvprintw(2,0, "Level: %d", new_pokemon.level);
    mvprintw(3,0, "--Stats--  HP:%d | ATK:%d | DEF:%d | SPD:%d | SA:%d | SD:%d", new_pokemon.get_hp(), new_pokemon.get_stat(2),  new_pokemon.get_stat(3), new_pokemon.get_stat(6),  new_pokemon.get_stat(4),  new_pokemon.get_stat(5));
    mvprintw(4,0, "--Genral INFO--  Species: %s | Gender: %c | Type: %s", species[new_pokemon.species_id].identifier, new_pokemon.gender, new_pokemon.shiny);

    mvprintw(6,0, "Number of moves: %d", new_pokemon.num_moves);


    if(new_pokemon.num_moves >= 1){
      mvprintw(7,0, "Move #1: %s", moves[new_pokemon.move1].identifier);
    }
    if(new_pokemon.num_moves == 2){
      mvprintw(8,0, "Move #2: %s",  moves[new_pokemon.move2].identifier);
    }
...
void io_battle(character *aggressor, character *defender)
{
  ...
  Pokemon_t p1;
  io_display();
  mvprintw(0,0,"%c wants a battle!", n->symbol);

  clear();
  mvprintw(0,0, "You have entered a battle with %c", n->symbol);
  mvprintw(1,0,"%c has a pokemon below:", n->symbol);
  mvprintw(2,0, "Level: %d", p1.level);
    mvprintw(3,0, "--Stats--  HP:%d | ATK:%d | DEF:%d | SPD:%d | SA:%d | SD:%d", p1.get_hp(), p1.get_stat(2),  p1.get_stat(3), p1.get_stat(6),  p1.get_stat(4),  p1.get_stat(5));
    mvprintw(4,0, "--Genral INFO--  Species: %s | Gender: %c | Type: %s", species[p1.species_id].identifier, p1.gender, p1.shiny);

    mvprintw(6,0, "Number of moves: %d", p1.num_moves);


    if(p1.num_moves >= 1){
      mvprintw(7,0, "Move #1: %s", moves[p1.move1].identifier);
    }
    if(p1.num_moves == 2){
      mvprintw(8,0, "Move #2: %s",  moves[p1.move2].identifier);
    }
