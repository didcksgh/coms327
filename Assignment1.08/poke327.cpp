...
int choose_starter_pokemon(){
  ...
  mvprintw(0,0,"Choose your pokemon!");

  //first pokemon
  mvprintw(1,0,"1. Lv. %d | %s", p1.level, p1.identifier);
  mvprintw(2,0,"--Stats--  HP:%d | ATK:%d | DEF:%d | SPD:%d | SA:%d | SD:%d", p1.get_hp(), p1.get_stat(2),  p1.get_stat(3), p1.get_stat(6),  p1.get_stat(4),  p1.get_stat(5));
  mvprintw(3,0,"--Genral INFO--  Species: %s | Gender: %c | Type: %s", species[p1.species_id].identifier, p1.gender, p1.shiny);
  mvprintw(4,0, "Number of moves: %d", p1.num_moves);


    if(p1.num_moves >= 1){
      mvprintw(5,0, "Move #1: %s", moves[p1.move1].identifier);
    }
    if(p1.num_moves == 2){
      mvprintw(6,0, "Move #2: %s",  moves[p1.move2].identifier);
    }
//second pokemon
    mvprintw(8,0,"2. Lv. %d | %s", p2.level, p2.identifier);
    mvprintw(9,0,"--Stats--  HP:%d | ATK:%d | DEF:%d | SPD:%d | SA:%d | SD:%d", p2.get_hp(), p2.get_stat(2),  p2.get_stat(3), p2.get_stat(6),  p2.get_stat(4),  p2.get_stat(5));
    mvprintw(10,0,"--Genral INFO--  Species: %s | Gender: %c | Type: %s", species[p2.species_id].identifier, p2.gender, p2.shiny);
    mvprintw(11,0, "Number of moves: %d", p2.num_moves);


    if(p2.num_moves >= 1){
      mvprintw(12,0, "Move #1: %s", moves[p2.move1].identifier);
    }
    if(p2.num_moves == 2){
      mvprintw(13,0, "Move #2: %s",  moves[p2.move2].identifier);
    }
//third pokemon
    mvprintw(15,0,"3. Lv. %d | %s", p3.level, p3.identifier);
    mvprintw(16,0,"--Stats--  HP:%d | ATK:%d | DEF:%d | SPD:%d | SA:%d | SD:%d", p3.get_hp(), p3.get_stat(2),  p3.get_stat(3), p3.get_stat(6),  p3.get_stat(4),  p3.get_stat(5));
    mvprintw(17,0,"--Genral INFO--  Species: %s | Gender: %c | Type: %s", species[p3.species_id].identifier, p3.gender, p3.shiny);
    mvprintw(18,0, "Number of moves: %d", p1.num_moves);


    if(p3.num_moves >= 1){
      mvprintw(19,0, "Move #1: %s", moves[p3.move1].identifier);
    }
    if(p3.num_moves == 2){
      mvprintw(20,0, "Move #2: %s",  moves[p3.move2].identifier);
    }

    mvprintw(22,0,"type the number of the pokemon you want to be a partner!");
    refresh();
    switch(getchar()){
      case '1' :
      id = p1.id;
...
void show_chosen_pokemon(int id){
  mvprintw(0,0,"You have chosen %s as a partner!", pokemon[id].identifier);
  refresh();
  getchar();
}
...
int get_distance(){
  int distance = (abs(world.cur_idx[dim_x] - (WORLD_SIZE / 2)) +
       abs(world.cur_idx[dim_y] - (WORLD_SIZE / 2)));
  return distance;
}
...
int getLevel(){
  ...
  if(distance<=200){
    level = rand() % ((distance/2) + 1 - 1) + 1;
  }
  else {
    level = rand() % (100 + 1 - ((distance-200)/2)) + ((distance-200)/2);
  }
  return level;
}
...
void Pokemon_t::set_moves(){
  ...
  if(found_moves.size() == 0){
    move1 = -1;
    move2 = -1;
    num_moves =0;
  }
  else if(found_moves.size() == 1){
    move1 = pokemon_moves[found_moves[0]].move_id;
    move2 = -1;
    num_moves =1;
  }
  else{
    int m1 = rand() % found_moves.size();
    move1 = pokemon_moves[found_moves[m1]].move_id;
    
    int m2 = rand() % found_moves.size();
    int a = 0;
    while(m1 == m2 && a <20){
      m2 = rand() % found_moves.size();
      a++;
    ...
    move2 = pokemon_moves[found_moves[m2]].move_id;
    num_moves=2;
    }
...
void Pokemon_t::set_base(){
  for (int i = 1; i < 6553; i++){
    if(pokemon_stats[i].pokemon_id == id){
      if(pokemon_stats[i].stat_id == 1){
        base_hp = pokemon_stats[i].base_stat;
      }
      if(pokemon_stats[i].stat_id == 2){
        base_attack = pokemon_stats[i].base_stat;
      }
      if(pokemon_stats[i].stat_id == 3){
        base_defense = pokemon_stats[i].base_stat;
      }
      if(pokemon_stats[i].stat_id == 4){
        base_special_attack = pokemon_stats[i].base_stat;
      }
      if(pokemon_stats[i].stat_id == 5){
        base_special_defense = pokemon_stats[i].base_stat;
      }
      if(pokemon_stats[i].stat_id == 6){
        base_speed = pokemon_stats[i].base_stat;
      }
    }
  }
}
...
void Pokemon_t::set_shiny(){
  if(rand() % 8192 == 0){
    strcpy(shiny, "Shiny");
  }
  else{
    strcpy(shiny, "Non-shiny");
  }
}

int Pokemon_t::get_hp(){
  int top = ((base_hp + iv_hp)*2)*level;
  int result = (top/100) + level + 10;
  return result;
}
...
int Pokemon_t::get_stat(int stat_id){
  ...
  case 4:
    ...
    iv = iv_special_attack;
    break;
  case 5:
    base = base_special_defense;
    iv = iv_special_defense;
    break;
  case 6:
    base = base_speed;
    iv = iv_speed;
...
int main(int argc, char *argv[])
{
  ...
  int id = choose_starter_pokemon();
  show_chosen_pokemon(id);
