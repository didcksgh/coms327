...
//move the pc to the next map randomly
uint32_t io_teleport_pc(pair_t dest)
{
  do {
      ...
      dest[dim_y] = rand_range(1, MAP_Y - 2);
             ...
             world.cur_map->map[dest[dim_y]][dest[dim_x]] != ter_path);
    

  return 0;
}

//flying to the other map
void io_fly_world(pair_t dest){
  int x,y;

  do {
    mvprintw(0,0,"Enter X-coordination[-200, 200]: ");
    refresh();
    echo();
    curs_set(1);
    mvscanw(0, 21, (char*)"%d", &x);

    mvprintw(0,0,"Enter Y-coordination[-200, 200]: ");
    refresh();
...
//prevent to go to the INT_MAX terrain
  if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == ter_gate && (input == 3 || input == 1 || input == 7 || input == 9 )) {
    return 1;
...
void io_handle_input(pair_t dest)
{
  ...
  do {
    ...
    case 't':
      ...
      break;
    case 'f':
      /* fly the PC to an assigned place in the map.              */
      io_fly_world(dest);
      turn_not_consumed = 0;
      break;
    case 'm':
